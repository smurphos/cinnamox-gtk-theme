...
if [[ $(date +"%m%d") = "0401" ]] && [[ ! $(echo "$@" | grep "no-jokes") ]] ; then
	...
	GTK3_GENERATE_DARK=False HDR_BTN_BG=f8f8f8 HDR_BTN_FG=000000 MENU_BG=ECE9D8
	MENU_FG=000000 OUTLINE_WIDTH=1 ROUNDNESS=3 SEL_BG=3169C6 SEL_FG=FFFFFF
...
CINN_BG=${CINN_BG-$MENU_BG}
CINN_FG=${CINN_FG-$MENU_FG}
NOTIFICATION_MARGIN_TOP=${NOTIFICATION_MARGIN_TOP-0}
NOTIFICATION_MARGIN_RIGHT=${NOTIFICATION_MARGIN_RIGHT-10}
...
if expr "$GTK2_GRAD_1" : '-\?[0-9]\+$' >/dev/null; then
  ...
  GTK2_SP_LARGE=0
  GTK2_SP_XLARGE=0
  GTK2HIDPI_SP_SMALL=1
  GTK2HIDPI_SP_MEDIUM=1
  GTK2HIDPI_SP_LARGE=1
  GTK2HIDPI_SP_XLARGE=1
  
  GTK2_SP_IND=15
  GTK2_SP_SLIDERW=9
  GTK2_SP_SLIDERL=60
  GTK2HIDPI_SP_IND=20
  GTK2HIDPI_SP_SLIDERW=12
  GTK2HIDPI_SP_SLIDERL=100
  
if (( SPACING > 2 )); then
  GTK2_SP_SMALL=$(( SPACING - 2 ))
  GTK2_SP_MEDIUM=$(( SPACING - 1 ))
  GTK2_SP_LARGE=$(( SPACING + 1 ))
  GTK2_SP_XLARGE=$(( SPACING + 3 ))
  GTK2HIDPI_SP_SMALL=$(( (( SPACING * 2 )) - 4 ))
  GTK2HIDPI_SP_MEDIUM=$(( (( SPACING * 2 )) - 2 ))
  GTK2HIDPI_SP_LARGE=$(( (( SPACING * 2 )) + 2 ))
  GTK2HIDPI_SP_XLARGE=$(( ((SPACING * 2 )) + 6 ))
elif (( SPACING > 1 )); then
  GTK2_SP_MEDIUM=1
  GTK2_SP_LARGE=2
  GTK2_SP_XLARGE=3
  GTK2HIDPI_SP_MEDIUM=2
  GTK2HIDPI_SP_LARGE=4
  GTK2HIDPI_SP_XLARGE=6
elif (( SPACING > 0 )); then
  GTK2_SP_LARGE=0
  GTK2_SP_XLARGE=1
  GTK2HIDPI_SP_LARGE=1
  GTK2HIDPI_SP_XLARGE=2
fi
if (( SPACING > 4 )); then
  GTK2HIDPI_SP_IND=25
  GTK2HIDPI_SP_SLIDERW=15
  GTK2HIDPI_SP_SLIDERL=120
elif (( SPACING > 9 )); then
  GTK2HIDPI_SP_IND=30
  GTK2HIDPI_SP_SLIDERW=20
  GTK2HIDPI_SP_SLIDERL=160
fi
elif (( SPACING > 14 )); then
  GTK2HIDPI_SP_IND=40
  GTK2HIDPI_SP_SLIDERW=30
  GTK2HIDPI_SP_SLIDERL=180
fi
...
SPECIAL_FG_COLOR=${SPECIAL_FG_COLOR:-$SEL_FG}
LINK_COLOR=${LINK_COLOR:-3f51b5}
ERROR_COLOR=${ERROR_COLOR:-f44336}
SUCCESS_COLOR=${SUCCESS_COLOR:-4caf50}
WARNING_COLOR=${WARNING_COLOR:-ef6c00}
INFO_BG_COLOR=${INFO_BG_COLOR:-03a9f4}

OUTPUT_THEME_NAME="${OUTPUT_THEME_NAME-cinnamox-$THEME}"
...
for FILEPATH in "${PATHLIST[@]}"; do
	find $(echo "${FILEPATH}" | sed -e 's/src\///g' ) -type f -exec sed -i'' \
		...
		-e 's/%CINN_BG%/'"$CINN_BG"'/g' \
		-e 's/%CINN_FG%/'"$CINN_FG"'/g' \
		-e 's/%BTN_BG%/'"$BTN_BG"'/g' \
		-e 's/%BTN_FG%/'"$BTN_FG"'/g' \
		...
		-e 's/%GTK2_SP_SMALL%/'"$GTK2_SP_SMALL"'/g' \
		-e 's/%GTK2_SP_MEDIUM%/'"$GTK2_SP_MEDIUM"'/g' \
		-e 's/%GTK2_SP_LARGE%/'"$GTK2_SP_LARGE"'/g' \
		-e 's/%GTK2_SP_XLARGE%/'"$GTK2_SP_XLARGE"'/g' \
		-e 's/%GTK2_SP_IND%/'"$GTK2_SP_IND"'/g' \
		-e 's/%GTK2_SP_SLIDERW%/'"$GTK2_SP_SLIDERW"'/g' \
		-e 's/%GTK2_SP_SLIDERL%/'"$GTK2_SP_SLIDERL"'/g' \
		-e 's/%GTK2HIDPI_SP_SMALL%/'"$GTK2HIDPI_SP_SMALL"'/g' \
		-e 's/%GTK2HIDPI_SP_MEDIUM%/'"$GTK2HIDPI_SP_MEDIUM"'/g' \
		-e 's/%GTK2HIDPI_SP_LARGE%/'"$GTK2HIDPI_SP_LARGE"'/g' \
		-e 's/%GTK2HIDPI_SP_XLARGE%/'"$GTK2HIDPI_SP_XLARGE"'/g' \
		-e 's/%GTK2HIDPI_SP_IND%/'"$GTK2HIDPI_SP_IND"'/g' \
		-e 's/%GTK2HIDPI_SP_SLIDERW%/'"$GTK2HIDPI_SP_SLIDERW"'/g' \
		-e 's/%GTK2HIDPI_SP_SLIDERL%/'"$GTK2HIDPI_SP_SLIDERL"'/g' \
		...
		-e 's/%NOTIFICATION_MARGIN_TOP%/'"$NOTIFICATION_MARGIN_TOP"'/g' \
		-e 's/%NOTIFICATION_MARGIN_RIGHT%/'"$NOTIFICATION_MARGIN_RIGHT"'/g' \
		...
		-e 's/%SPECIAL_FG_COLOR%/'"$SPECIAL_FG_COLOR"'/g' \
		-e 's/%LINK_COLOR%/'"$LINK_COLOR"'/g' \
		-e 's/%ERROR_COLOR%/'"$ERROR_COLOR"'/g' \
